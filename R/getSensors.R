...
##' @title getSensorTypes
##'
##' @description Retrieves the sensors types
##' @return WSResponse object
##' @seealso http://docs.brapi.apiary.io/#introduction/url-structure
##' @seealso You have to install the opensilexWSClientR before running any 
##'          request on PHIS web service.
...
##'                username="guest@opensilex.org",
##'                password="guest")
##' sensors <- getSensorTypes()
##' sensors$data
##' }
##' @export
getSensorTypes <- function(){
  
  attributes <- list(pageSize = 1000,page = 0) 
  
  uriEncoded<-paste0(utils::URLencode("http://www.opensilex.org/vocabulary/oeso#SensingDevice",  reserved = TRUE),"/descendants")
  Response <- opensilexWSClientR::getResponseFromWS(resource = paste0(get("URI", configWS)), paramPath = uriEncoded,
                                                    attributes = attributes, wsVersion = 2)
  return(Response)
} 
